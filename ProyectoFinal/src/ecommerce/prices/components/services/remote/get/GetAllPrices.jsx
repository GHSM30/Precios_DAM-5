  ...
  return new Promise((resolve, reject) => {
    ...
    axios.get(apiUrl)
      .then((response) => {
        ...
        if (!data.success) {
          console.error("No se pudo realizar correctamente la petición <<getAllPrices - Services>>", data);
          reject(data); // Enviar el objeto de error como rechazo
        } else {
          console.log("Precios obtenidos:", data.data);
          resolve(data.data); // Solo devolver el array necesario de los precios
        }
      })
      .catch((error) => {
